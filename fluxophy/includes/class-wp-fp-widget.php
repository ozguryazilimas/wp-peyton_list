...
function fluxophy_display_data($source_url, $display_count, $picture_url, $account_name, $account_link, $twitter_account, $twitter_data_id) {
  ...
  $output .= '<div id="fluxophy_button_show_twitter" class="active">';
  ...
  $output .= '<div id="fluxophy_widget_twitter" style="">';
  ...
  if ($request_result) {
    ...
    for ($i = 0; $i < $display_count; $i++) {
      if (isset($received->data[$i]->message)) {
        ...
        if (isset($received->data[$i]->id)) {
          $fb_post_url = 'https://www.facebook.com/' . $received->data[$i]->id;
        } else {
          $fb_post_url = '';
        }
        ...
        $output .= '<a href="' . $fb_post_url . '" title="' . date_i18n('G:i - d F Y', strtotime($received->data[$i]->created_time)) . '" target="_blank">';
