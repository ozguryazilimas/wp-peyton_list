...
if( !class_exists('Wbcr_FactoryPages479_AdminPage') ) {

	class Wbcr_FactoryPages479_AdminPage extends Wbcr_FactoryPages479_Page {
		...
		public function connect()
		{
			...
			$this->hidden = apply_filters('wbcr_factory_479_page_is_hidden_' . $result_id, $this->hidden);

			if( $this->hidden ) {
				return;
			}

			$this->internal = apply_filters('wbcr_factory_479_page_is_internal_' . $result_id, $this->internal);
			...
			if( $this->request->get('page', 'none') == $result_id ) {
				...
				if( !$this->scripts->isEmpty('bootstrap') || !$this->styles->isEmpty('bootstrap') ) {
					add_action('wbcr_factory_479_bootstrap_enqueue_scripts_' . $this->plugin->getPluginName(), [
						$this,
