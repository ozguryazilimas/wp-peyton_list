 ...
 */
function wfactory_479_install_components($plugin_instance)
{
	...
	if( !current_user_can('update_plugins') ) {
		wp_die(__('You don\'t have enough capability to edit this information.', 'wbcr_factory_479'), __('Something went wrong.'), 403);
	}

	if( empty($slug) || empty($action) ) {
		wp_send_json_error(['error_message' => __('Required attributes are not passed or empty.', 'wbcr_factory_479')]);
	}
	$success = false;
	$send_data = [];
	...
	if( $storage == 'internal' ) {
		if( $action == 'activate' ) {
			...
			wp_send_json_error(['error_message' => __('You are trying to perform an invalid action.', 'wbcr_factory_479')]);
		}
	...
	wp_send_json_error(['error_message' => __('An unknown error occurred during the activation of the component.', 'wbcr_factory_479')]);
}
 ...
 */
function wfactory_479_prepare_component($plugin_instance)
{
	check_ajax_referer('updates');
	...
	if( !current_user_can('update_plugins') ) {
		wp_send_json_error(['error_message' => __('You don\'t have enough capability to edit this information.', 'wbcr_factory_479')], 403);
	}

	if( empty($component_name) ) {
		wp_send_json_error(['error_message' => __('Required attribute [component_name] is empty.', 'wbcr_factory_479')]);
	}
	// todo: для совместимости с плагином Clearfy
	if( "wbcr_clearfy" === $plugin_instance->getPluginName() ) {
 ...
 */
function wfactory_479_creativemotion_install_plugin($plugin_instance)
{
	check_ajax_referer('updates');
