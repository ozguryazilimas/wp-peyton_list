<?php
	...
	if( !class_exists('Wbcr_FactoryForms479_CheckboxControl') ) {

		class Wbcr_FactoryForms479_CheckboxControl extends Wbcr_FactoryForms479_Control {
			...
			protected function buttonsHtml()
			{
				...
				<div <?php $this->attrs() ?>>
					<button type="button" class="btn btn-default btn-small btn-sm factory-on <?php if( $value ) {
						echo 'active';
					} ?>"><?php _e('On', 'wbcr_factory_forms_479') ?></button>
					<button type="button" class="btn btn-default btn-small btn-sm factory-off <?php if( !$value ) {
						echo 'active';
					} ?>" data-value="0"><?php _e('Off', 'wbcr_factory_forms_479') ?></button>
