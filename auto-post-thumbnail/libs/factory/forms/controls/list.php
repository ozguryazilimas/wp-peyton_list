<?php
	...
	if( !class_exists('Wbcr_FactoryForms479_ListControl') ) {

		class Wbcr_FactoryForms479_ListControl extends Wbcr_FactoryForms479_Control {
				...
				if( $this->isAjax() ) {
					...
					<script>
						window['<?php echo esc_attr($ajax_id); ?>'] = {
							...
							'emptyList': '<?php echo esc_html($this->getOption('empty', __('The list is empty.', 'wbcr_factory_forms_479') )); ?>'
						};
					</script>
					<?php
			...
			protected function checklistHtml()
			{
				...
				$emptyList = $this->getOption('empty', __('The list is empty.', 'wbcr_factory_forms_479'));
