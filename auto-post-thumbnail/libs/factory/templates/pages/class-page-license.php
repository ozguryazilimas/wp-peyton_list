...
class License extends PageBase {
	...
	/**
	 * @since 2.0.7
	 * @var \WBCR\Factory_479\Premium\Interfaces\License
	 */
	...
	public function assets($scripts, $styles)
	{
		...
		$this->styles->add(FACTORY_TEMPLATES_132_URL . '/assets/css/license-manager.css');
		$this->scripts->add(FACTORY_TEMPLATES_132_URL . '/assets/js/clearfy-license-manager.js');
	}
	...
	{
		wbcr_factory_templates_132_check_license($this->plugin);
	}
	...
	public function showPageContent()
	{
		...
		<div id="wcl-license-wrapper"
		     data-loader="<?php echo FACTORY_TEMPLATES_132_URL . '/assets/img/loader.gif'; ?>"
	...
	protected function render_learnmore_section()
	{
		if( $this->is_premium ):
			...
			<p style="margin-top: 10px;">
				<?php printf(__('<a href="%s" target="_blank" rel="noopener">Lean more</a> about the premium version and get the license key to activate it now!', 'wbcr_factory_templates_132'), $this->plugin->get_support()->get_pricing_url(true, 'license_page')); ?>
			</p>
		<?php else: ?>
			<p style="margin-top: 10px;">
				<?php printf(__('Can’t find your key? Go to <a href="%s" target="_blank" rel="noopener">this page</a> and login using the e-mail address associated with your purchase.', 'wbcr_factory_templates_132'), $this->plugin->get_support()->get_contacts_url(true, 'license_page')) ?>
			</p>
		<?php endif;
	}
	...
	public function show_license_form($notice = false)
	{
		...
		<div id="license-manager"
		     class="factory-bootstrap-481 onp-page-wrap <?php echo $this->get_license_type() ?>-license-manager-content">
			<div>
				<h3><?php printf(__('Activate %s', 'wbcr_factory_templates_132'), $this->plan_name) ?></h3>
			...
			<?php if( is_wp_error($notice) ) : ?>
				<div class="license-message <?php echo $this->get_license_type() ?>-license-message">
					<div class="alert <?php echo esc_attr($notice->get_error_code()); ?>">
						<h4 class="alert-heading"><?php _e($notice->get_error_message(), 'wbcr_factory_templates_132') ?></h4>
					</div>
				</div>
			<?php endif; ?>
			<div class="onp-container">
				<div class="license-details">
					<?php if( $this->get_license_type() == 'free' ): ?>
						<a href="<?php echo $this->plugin->get_support()->get_pricing_url(true, 'license_page'); ?>"
						   class="purchase-premium" target="_blank" rel="noopener">
                            ...
                            <?php _e('Upgrade to Premium', 'wbcr_factory_templates_132') ?>
                            </span>
						</a>
						<p><?php printf(__('Your current license for %1$s:', 'wbcr_factory_templates_132'), $this->plugin->getPluginTitle()) ?></p>
					<?php endif; ?>
					<div class="license-details-block <?php echo $this->get_license_type() ?>-details-block">
						...
						<div class="license-key-description">
							<?php if( !$this->is_premium ): ?>
								<p><?php _e('Public License is a GPLv3 compatible license allowing you to change and use this version of the plugin for free. Please keep in mind this license covers only free edition of the plugin. Premium versions are distributed with other type of a license.', 'wbcr_factory_templates_132') ?></p>
							<?php else: ?>
								<p><?php _e('Сommercial license, only to the premium add-on to this free plugin. You cannot distribute or modify the premium add-on. But free plugin is a GPLv3 compatible license allowing you to change and use this version of the plugin for free.', 'wbcr_factory_templates_132') ?></p>
							<?php endif; ?>
							<?php if( $this->is_premium && $this->premium_has_subscription ): ?>
								<p class="activate-trial-hint">
									<?php _e('You use a paid subscription for the plugin updates. In case you don’t want to receive paid updates, please, click <a data-action="unsubscribe" class="wcl-control-btn" href="#">cancel subscription</a>', 'wbcr_factory_templates_132') ?>
								</p>
							<?php endif; ?>

							<?php if( $this->get_license_type() == 'trial' ): ?>
								<p class="activate-error-hint">
									<?php printf(__('Your license has expired, please extend the license to get updates and support.', 'wbcr_factory_templates_132'), '') ?>
								</p>
							<?php endif; ?>
						</div>
						<table class="license-params" colspacing="0" colpadding="0">
							<tr>
								<!--<td class="license-param license-param-domain">
										...
										<span class="license-value-name"><?php _e('domain', 'wbcr_factory_templates_132') ?></span>
									</td>-->
								<td class="license-param license-param-days">
									<span class="license-value"><?php echo $this->get_plan() ?></span>
									<span class="license-value-name"><?php _e('plan', 'wbcr_factory_templates_132') ?></span>
								</td>
								<?php if( $this->is_premium ) : ?>
									<td class="license-param license-param-sites">
                                        <span class="license-value">
                                            <?php echo esc_attr($this->premium_license->get_count_active_sites()); ?>
                                            <?php _e('of', 'wbcr_factory_templates_132') ?>
                                            <?php echo esc_attr($this->premium_license->get_sites_quota()); ?></span>
										<span class="license-value-name"><?php _e('active sites', 'wbcr_factory_templates_132') ?></span>
									</td>
								<?php endif; ?>
								...
								<?php if( $this->is_premium ): ?>
									<td class="license-param license-param-days">
										<?php if( $this->get_license_type() == 'trial' ): ?>
											<span class="license-value"><?php _e('EXPIRED!', 'wbcr_factory_templates_132') ?></span>
											<span class="license-value-name"><?php _e('please update the key', 'wbcr_factory_templates_132') ?></span>
										<?php else: ?>
											<span class="license-value">
													<?php
													...
													?>
                                                <small> <?php _e('day(s)', 'wbcr_factory_templates_132') ?></small>
                                             </span>
											<span class="license-value-name"><?php _e('remained', 'wbcr_factory_templates_132') ?></span>
										<?php endif; ?>
									</td>
								<?php endif; ?>
							</tr>
				...
				<div class="license-input">
					<form action="" method="post">
						<?php if($this->is_premium): ?>
					<p><?php _e('Have a key to activate the premium version? Paste it here:', 'wbcr_factory_templates_132') ?><p>
					<?php else: ?>
						<p><?php _e('Have a key to activate the plugin? Paste it here:', 'wbcr_factory_templates_132') ?>
						<p>
							<?php endif; ?>
							<button data-action="activate" class="btn btn-default wcl-control-btn" type="button"
							        id="license-submit">
								<?php _e('Submit Key', 'wbcr_factory_templates_132') ?>
							</button>
