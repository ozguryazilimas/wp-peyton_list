...
class Components extends \WBCR\Factory_Templates_132\Pages\PageBase {
	...
	{
		$this->menu_title = __('Components', 'wbcr_factory_templates_132');
		$this->page_menu_short_description = __('More features for plugin', 'wbcr_factory_templates_132');
	...
	public function getActionNotices($notices)
	{
		$notices[] = [
			...
			'type' => 'success',
			'message' => __('Components have been successfully updated to the latest version.', 'wbcr_factory_templates_132')
		];
		...
		$notices[] = [
			...
			'type' => 'danger',
			'message' => __('To use premium components, you need activate a license!', 'wbcr_factory_templates_132') . '<a href="admin.php?page=license-wbcr_clearfy" class="btn btn-gold">' . __('Activate license', 'wbcr_factory_templates_132') . '</a>'
		];
	...
	public function showPageContent()
	{
		...
		?>
		<div class="wbcr-factory-page-group-header"><?php _e('<strong>Plugin Components</strong>.', 'wbcr_factory_templates_132') ?>
			<p>
				<?php _e('These are components of the plugin bundle. When you activate the plugin, all the components turned on by default. If you don’t need some function, you can easily turn it off on this page.', 'wbcr_factory_templates_132') ?>
			</p>
		</div>
					...
					<div class="plugin-card-bottom">
						<?php if( !(isset($component['disable_button']) && $component['disable_button']) ): ?>
							<?php if( 'premium' === $component['build'] && !($this->plugin->premium->is_activate() && $this->plugin->premium->is_install_package()) ): ?>
								<a target="_blank" href="<?php echo esc_url($component['url']) ?>" class="button button-default install-now"><?php _e('Read more', 'wbcr_factory_templates_132'); ?></a>
							<?php else: ?>
