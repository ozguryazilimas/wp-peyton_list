...
namespace WBCR\Factory_Templates_132;
...
if ( ! class_exists( 'WBCR\Factory_Templates_132\Impressive' ) ) {
	/**
	 * Class Wbcr_FactoryPages479_ImpressiveThemplate
	 */
	abstract class Impressive extends \Wbcr_FactoryPages479_AdminPage {
		...
		public function __construct( \Wbcr_Factory479_Plugin $plugin ) {
			$this->menu_icon = FACTORY_TEMPLATES_132_URL . '/pages/templates/impressive/assets/img/webcraftic-plugin-icon.png';
			//$allow_multisite = apply_filters('wbcr_factory_479_core_admin_allow_multisite', false);
			...
			$this->title_plugin_action_link = __( 'Settings', 'wbcr_factory_pages_479' );
		...
		public function assets( $scripts, $styles ) {
			...
			$this->styles->add( FACTORY_TEMPLATES_132_URL . '/pages/templates/impressive/assets/css/impressive.page.template.css' );
		}
		...
		public function flushCacheAndRulesAction() {
			...
			if ( class_exists( 'WBCR\Factory_Templates_132\Helpers' ) ) {
		...
		protected function warningNotice() {
			/*if( WP_CACHE ) {
				$this->printWarningNotice(__("It seems that a caching/performance plugin is active on this site. Please manually invalidate that plugin's cache after making any changes to the settings below.", 'wbcr_factory_pages_479'));
			}*/
		...
		private function showActionsNotice() {
			$notices = [
				[
					...
					'type'       => 'success',
					'message'    => __( 'The settings have been updated successfully!', 'wbcr_factory_pages_479' ) . ( WP_CACHE ? '<br>' . __( "It seems that a caching/performance plugin is active on this site. Please manually invalidate that plugin's cache after making any changes to the settings below.", 'wbcr_factory_pages_479' ) : '' )
				]
			];
                ...
                <div class="wbcr-factory-control">
					...
					<?php if ( $this->type == 'options' ): ?>
                        <input name="<?php echo $this->plugin->getPluginName() ?>_save_action"
                               ...
                               value="<?php _e( 'Save', 'wbcr_factory_pages_479' ); ?>">
		...
		protected function showOptions() {
			$form = new \Wbcr_FactoryForms479_Form( [
				'scope' => rtrim( $this->plugin->getPrefix(), '_' ),
				'name'  => $this->getResultId() . "-options"
			], $this->plugin );

			$form->setProvider( new \Wbcr_FactoryForms479_OptionsValueProvider( $this->plugin ) );
			...
			if ( isset( $_POST[ $this->plugin->getPluginName() . '_save_action' ] ) ) {
				...
				if ( ! current_user_can( 'administrator' ) && ! current_user_can( $this->capabilitiy ) ) {
					wp_die( __( 'You do not have permission to edit page.', 'wbcr_factory_pages_479' ) );
				}
            ...
            <div id="WBCR" class="wrap">
                <div class="wbcr-factory-templates-132-impressive-page-template factory-bootstrap-481 factory-fontawesome-000">
