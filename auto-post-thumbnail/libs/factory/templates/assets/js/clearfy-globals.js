 ...
 * и самого Clearfy.
 *
 * $.wbcr_factory_templates_132.app - методы для работы с приложением. Скрыть, показать уведомления.
 * $.wbcr_factory_templates_132.hooks - это иммитация хуков и фильтров аналогично тем, что используются в Wordpress
 *
...
(function($) {
	...
	if( !$.wbcr_factory_templates_132 ) {
		$.wbcr_factory_templates_132 = {};
	}

	//todo: Переопредление для совместимости со старыми версиями плагинов.
	$.wbcr_factory_templates_132.filters = $.wbcr_factory_templates_132.filters || $.wfactory_479.filters;
	//todo: Переопредление для совместимости со старыми версиями плагинов.
	$.wbcr_factory_templates_132.hooks = $.wbcr_factory_templates_132.hooks || $.wfactory_479.hooks;

	$.wbcr_factory_templates_132.app = $.wbcr_factory_templates_132.app || {
		/**
		...
		showNotice: function(message, type) {
			...
			$([document.documentElement, document.body]).animate({
				 ...
				 * Пример регистрации хука $.wfactory_479.hooks.add('wbcr/factory_templates_132/updated',
				 * function(noticeId) {});
				 * @param {string} noticeId - id уведомления
				 */
				$.wfactory_479.hooks.run('wbcr/factory_templates_132/showed_notice', [noticeId]);
				$.wfactory_479.hooks.run('wbcr/clearfy/showed_notice', [noticeId]);
			});
		...
		hideNotice: function(noticeId) {
			...
			el.fadeOut(500, function(e) {
				 ...
				 * @param {string} noticeId - id уведомления
				 */
				$.wfactory_479.hooks.run('wbcr/factory_templates_132/hidded_notice', [noticeId]);
				$.wfactory_479.hooks.run('wbcr/clearfy/hidded_notice', [noticeId]);
			});
		},
	...
	$.wfactory_479.hooks.add('core/components/deactivated', function(button, data, response) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !$('#WBCR').length ) {
			return false;
		}
	...
	$.wfactory_479.hooks.add('core/components/deleted', function(button) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !$('#WBCR').length ) {
			return false;
		}
	...
	$.wfactory_479.hooks.add('core/components/activation_error', function(plugin, button, response) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		...
		if( response.data && response.data.error_message ) {
			$.wbcr_factory_templates_132.app.showNotice(response.data.error_message, 'danger');
		}
	});

	$.wfactory_479.hooks.add('core/components/update_error', function(button, data, response) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !($('#WBCR').length && $.wbcr_factory_templates_132) ) {
			return false;
		}
	...
	$.wfactory_479.hooks.add('core/components/activated', function(button, data, response) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !$('#WBCR').length ) {
			return false;
		}
	...
	$.wfactory_479.hooks.add('core/components/ajax_error', function(xhr, ajaxOptions, thrownError) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !($('#WBCR').length && $.wbcr_factory_templates_132) ) {
			return false;
		}
